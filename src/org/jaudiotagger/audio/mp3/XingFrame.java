...
public class XingFrame
{
    ...
    private static final int XING_TOC_BUFFER_SIZE = 100;    
    private static final int XING_QUALITY_BUFFER_SIZE = 4;
    ...
    private XingFrame(ByteBuffer header)
    {
		...
		if ((flagBuffer[BYTE_4] & (byte) (1 << 2)) != 0)
		{
			setTOC();	
		}        
        
        if ((flagBuffer[BYTE_4] & (byte) (1 << 3)) != 0)
        {
        	setQuality();
        }
    ...
    private void setTOC()
    {
        byte frameSizeBuffer[] = new byte[XING_TOC_BUFFER_SIZE];
        header.get(frameSizeBuffer);        
    }    
    
    
    /**
     * Set lame encoder quality
     */
    private void setQuality()
    {
        byte frameSizeBuffer[] = new byte[XING_QUALITY_BUFFER_SIZE];
        header.get(frameSizeBuffer);
        quality  = (frameSizeBuffer[0] << 24)&0xFF000000 | (frameSizeBuffer[1] << 16)&0x00FF0000 | (frameSizeBuffer[2] << 8)&0x0000FF00 | frameSizeBuffer[3]&0x000000FF;
